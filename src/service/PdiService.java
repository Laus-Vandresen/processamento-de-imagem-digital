...
import auxiliar.Pixel;
import auxiliar.Vizinhos;
...
public class PdiService {
	...
	public static Image DividirImagemEm2(Image imagem) {
		int w = (int) imagem.getWidth();
		int h = (int) imagem.getHeight();
	...
	private static int[] histogramaTodos(Image image, String op) {
	...
	private static int qtTons(int[] histogram) {
		int qt = 255;
	...
	private static int minPoint(int[] histogram) {
		for (int i = 0; i < histogram.length; i++) {
			if (histogram[i] > 0) {
				return i;
			}
	...
	private static int[] histogramaAcumulado(int[] histogram) {
		int total = 0;
		int[] histogramAc = new int[histogram.length];
		for (int i = 0; i < histogramAc.length; i++) {
			histogramAc[i] = total + histogram[i];
			total = histogramAc[i];
		}
		return histogramAc;
	}

	public static Image equalizacao(Image img, boolean todos) {
		int w = (int) img.getWidth();
		int h = (int) img.getHeight();
		...
		int[] hR = histogramaTodos(img, "RED");
		int[] hG = histogramaTodos(img, "GREEN");
		int[] hB = histogramaTodos(img, "BLUE");

		int[] accHistR = histogramaAcumulado(hR);
		int[] accHistG = histogramaAcumulado(hG);
		int[] accHistB = histogramaAcumulado(hB);

		int qtTonsRed = qtTons(hR);
		int qtTonsGreen = qtTons(hG);
		int qtTonsBlue = qtTons(hB);

		int minR = minPoint(hR);
		int minG = minPoint(hG);
		int minB = minPoint(hB);
		...
		for (int i = 1; i < w; i++) {
			for (int j = 1; j < h; j++) {
				...
				double acR = accHistR[(int) (oldColor.getRed() * 255)];
				double acG = accHistG[(int) (oldColor.getGreen() * 255)];
				double acB = accHistB[(int) (oldColor.getBlue() * 255)];
