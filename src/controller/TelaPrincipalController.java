...
public class TelaPrincipalController {
	...
	public void cinzaAritmetica() {
		img3 = PdiService.cinzaMediaAritmética(img1, 0, 0, 0);
		atualizaImagem3();
	}
	...
	public void cinzaPonderada() {
		...
		img3 = PdiService.cinzaMediaAritmética(img1, ipctR, ipctG, ipctB);
		atualizaImagem3();
	}
	...
	public void limirizar() {
		img3 = PdiService.limiarizacao(img1, slider.getValue() / 100);
		atualizaImagem3();
	}

	@FXML
	public void negativar() {
		img3 = PdiService.negativacao(img1);
		atualizaImagem3();
	}

	@FXML
	public void adicao() {
		img3 = PdiService.adicao(img1, img2, adiSubSlider1.getValue(), adiSubSlider2.getValue());
		atualizaImagem3();
	}

	@FXML
	public void subtracao() {
		img3 = PdiService.subtracao(img1, img2);
		atualizaImagem3();
	}
	...
	public void ruido() {
		...
		if (viz3.isSelected())
			...
			vizinho = 2;
		else if (vizX.isSelected())
			vizinho = 3;

		img3 = PdiService.ruidos(img1, vizinho);
		atualizaImagem3();
	}
	...
	public void MarcacaoMouseRelease(MouseEvent evento) {
		...
		if (img.getImage() != null) {
			xf = (int) evento.getX();
			yf = (int) evento.getY();
			img.setImage(PdiService.marcacao(img3, Math.min(xi, xf), Math.min(yi, yf), Math.max(xi, xf), Math.max(yi, yf)));
		}
	...
	public void abreHistograma(ActionEvent event) {
		try {
			...
			if (img1 != null) {
				PdiService.montaGrafico(img1, controller.grafico1);
			}
			if (img2 != null) {
				PdiService.montaGrafico(img2, controller.grafico2);
			}
			if (img3 != null) {
				PdiService.montaGrafico(img3, controller.grafico3);
			}
	...
	@FXML
	private void equalizacaoTodos() {
		img3 = PdiService.equalizacao(img1, true);
		atualizaImagem3();
	}

	@FXML
	private void equalizacao() {
		img3 = PdiService.equalizacao(img1, false);
		atualizaImagem3();
	}

	@FXML
	private void divideImagemEm2() {
		img3 = PdiService.DividirImagemEm2(img1);
		atualizaImagem3();
	}
